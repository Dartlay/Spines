...
class ReviewService {
  ...
  static Future<void> saveReview(String bookId, Review review) async {
    try {
  ...
  static Future<List<Review>> loadReviews(String bookId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final allReviews = prefs.getString(_reviewsKey) ?? '{}';
      final Map<String, dynamic> reviewsMap = json.decode(allReviews);
  ...
  static Future<Map<String, List<Review>>> loadAllReviews() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final allReviews = prefs.getString(_reviewsKey) ?? '{}';
      final Map<String, dynamic> reviewsMap = json.decode(allReviews);
